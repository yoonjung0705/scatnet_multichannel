...
file_names_data = glob.glob(os.path.join(root_dir, 'ad57_*.txt'))
...
for file_name_data in file_names_data:
    data = pd.read_csv(file_name_data, header=None, delimiter='\t')
...
for file_name_data in file_names_data:
    data = pd.read_csv(file_name_data, header=None, delimiter='\t')
    data = (data.loc[:data_len_total, [0, 1]].values - data.loc[:data_len_total, [2, 3]].values).T # shaped (2, data_len_total)
    data = data.reshape([2, -1, data_len]).transpose([1, 0, 2]) # shaped [n_data, 2, data_len]
    datas.append(data)
    labels.append(k)
