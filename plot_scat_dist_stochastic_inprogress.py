...
# Initial values of x.
x[:, 0] = 50
r = norm.rvs(size=(N,), scale=delta*sqrt(dt))
# y_brownian = brownian(x[:,0], N, dt, delta, out=x[:,1:])[0]
...
ax2.plot(dilation[:-2], diff_scat_arr[:-2], label='Scattering')
...
ax2.plot(dilation[:-2], diff_sig_arr_rescaled[:-2], label='L2 rescaled')
...
ax2.plot(dilation[:-2], diff_fourier_arr_rescaled[:-2], label='Fourier modulus L2 rescaled')
