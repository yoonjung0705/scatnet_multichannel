'''module that processes irfp data prior to training with length being equal for all trajectories'''
import os
import numpy as np
import pandas as pd
import re
...
data_len = min(track_lens)
print("Truncating all trajectories to length {}".format(data_len))
labels = np.concatenate(labels, axis=0)
index = nu._train_test_split(n_tracks_total, train_ratio=train_val_ratio, seed=42)
data_train = [data[idx][:, :data_len] for idx in index['train']]
data_test = [data[idx][:, :data_len] for idx in index['test']]
